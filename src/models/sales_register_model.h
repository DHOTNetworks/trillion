...
class SalesRegisterModel : public QAbstractListModel {
    Q_OBJECT
...
public:
    ...
    explicit SalesRegisterModel(QObject* parent = nullptr);

    int rowCount(const QModelIndex& parent = QModelIndex()) const override;
    QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const override;
    QHash<int, QByteArray> roleNames() const override;

    void setEntries(const QVector<SalesRegisterEntry>& entries);
    void clear();
...
private:
    QVector<SalesRegisterEntry> m_entries;
};

class SalesRegisterController : public QObject {
    Q_OBJECT
    Q_PROPERTY(SalesRegisterModel* model READ model CONSTANT)
...
public:
    explicit SalesRegisterController(QObject* parent = nullptr);

    SalesRegisterModel* model() { return &m_model; }
...
private:
    ...
    SalesRegisterModel m_model;
    QVector<SalesRegisterEntry> m_allEntries;
