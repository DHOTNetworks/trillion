#include "sales_register_model.h"
...
SalesRegisterModel::SalesRegisterModel(QObject* parent)
    : QAbstractListModel(parent)
{
}

int SalesRegisterModel::rowCount(const QModelIndex& parent) const {
    if (parent.isValid()) return 0;
    return m_entries.size();
}

QVariant SalesRegisterModel::data(const QModelIndex& index, int role) const {
    ...
    case CustValRole: return item.custVal;
...
void SalesRegisterModel::setEntries(const QVector<SalesRegisterEntry>& entries) {
    beginResetModel();
    m_entries = entries;
    endResetModel();
    emit countChanged();
...
QVariantMap SalesRegisterModel::get(int row) const {
...
SalesRegisterController::SalesRegisterController(QObject* parent)
    : QObject(parent)
{
}

void SalesRegisterController::setSearchQuery(const QString& query) {
...
void SalesRegisterController::reload(const QString& fromDate, const QString& toDate) {
    ...
    if (!fDate.isEmpty() && !tDate.isEmpty() && fDate != "ALL" && fDate != "All") {
        sql = "SELECT id, voucher_no, invoice_no, invoice_date, customer_name, item_name, bag_count, weight_qtl, "
...
void SalesRegisterController::applyFilter() {
    QString q = m_searchQuery.trimmed().toLower();
    QVector<SalesRegisterEntry> filtered;
