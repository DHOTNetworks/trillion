...
bool PurchaseVoucherController::loadInvoiceForEditing(const QVariant &invNoOrId, const QString &dateHint)
{
    QString key = invNoOrId.toString().trimmed();
    if (key.isEmpty()) return false;

    PurchaseModel purchaseModel;
    QVariantMap inv = purchaseModel.get_purchase_invoice(key, dateHint);
    if (inv.isEmpty() || !inv.contains("id") || inv.value("id").toInt() <= 0) {
        m_statusMessage = "Purchase Invoice not found";
        m_isError = true;
        emit statusChanged();
        return false;
    }
