...
bool PurchaseModel::add_purchase_invoice_full(
    ...
    // Debit: Purchase Account for Taxable Amount + GST Input Accounts
...
QVariantMap PurchaseModel::get_purchase_invoice(const QVariant& invoiceNoOrId, const QString& dateHint, const QString& partyHint) {
    ...
    if (rows.isEmpty() && !q.isEmpty()) {
        QVariantList vRows = DatabaseManager::instance().executeQuery(
            ...
            "FROM vouchers WHERE (id = ? OR voucher_no = ? OR ref_no = ?) AND (voucher_type IN ('Purchase', 'Purc') OR legacy_type IN ('Purc', 'Purchase')) LIMIT 1;",
            {q, q, q}
        );
        if (!vRows.isEmpty()) {
            ...
            if (rows.isEmpty()) {
                ...
                QVariantList stRows = DatabaseManager::instance().executeQuery(
                    ...
                    "FROM stock_transactions WHERE (voucher_no = ? OR bill_no = ? OR (trans_date = ? AND party_name = ?)) AND trans_type IN ('Purc', 'Purchase') "
