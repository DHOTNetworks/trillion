...
SalesRegisterWidget::SalesRegisterWidget(SalesRegisterController* controller, PrintExportController* printExportCtrl, QWidget* parent)
    : QWidget(parent)
...
void SalesRegisterWidget::setupUi() {
    ...
    auto* titleLabel = new QLabel("Sales Invoices Register (Commercial Sales Bills)", headerCard);
    titleLabel->setStyleSheet("font-size: 16px; font-weight: 800; color: #0F172A; border: none; background: transparent;");
    auto* subLabel = new QLabel("Complete audit ledger of customer invoices, rice & byproduct dispatch billing, and output GST.", headerCard);
    ...
    auto* newInvBtn = new KbdBadgeButton("+ New Sales Invoice", "F8", headerCard);
    newInvBtn->setPrimaryColor("#16A34A", "#15803D");
    newInvBtn->setTextColor("#FFFFFF");
    connect(newInvBtn, &QPushButton::clicked, this, &SalesRegisterWidget::newInvoiceRequested);
    headerLayout->addWidget(newInvBtn);

    auto* printBtn = new KbdBadgeButton("Print Invoice", "Ctrl+P", headerCard);
    printBtn->setPrimaryColor("#059669", "#047857");
    printBtn->setTextColor("#FFFFFF");
    connect(printBtn, &QPushButton::clicked, this, &SalesRegisterWidget::onPrintInvoice);
    ...
    connect(pdfBtn, &QPushButton::clicked, this, &SalesRegisterWidget::onExportPdf);
    ...
    connect(csvBtn, &QPushButton::clicked, this, &SalesRegisterWidget::onExportCsv);
    ...
    connect(backBtn, &QPushButton::clicked, this, &SalesRegisterWidget::backRequested);
    ...
    connect(m_fromDateEdit, &QDateEdit::dateChanged, this, &SalesRegisterWidget::onDateFilterChanged);
    ...
    connect(m_toDateEdit, &QDateEdit::dateChanged, this, &SalesRegisterWidget::onDateFilterChanged);
    ...
    m_searchEdit->setPlaceholderText("Search invoice no, buyer, commodity, vehicle...");
    m_searchEdit->setClearButtonEnabled(true);
    connect(m_searchEdit, &QLineEdit::textChanged, this, &SalesRegisterWidget::onSearchChanged);
    ...
    m_table->setHorizontalHeaderLabels({
        "Invoice No", "Date", "Customer / Buyer", "Commodity / Item",
    ...
    m_totalInvoicesLabel = createMetricCard("TOTAL INVOICES", "0", "#2563EB");
    ...
    m_totalGstLabel = createMetricCard("TOTAL GST", "₹ 0.00", "#059669");
    m_totalGrossLabel = createMetricCard("GROSS REVENUE", "₹ 0.00", "#16A34A");
...
void SalesRegisterWidget::populateTable() {
    ...
    QString sql = "SELECT id, voucher_no, invoice_no, invoice_date, customer_name, item_name, "
                  ...
                  "FROM sales_invoices WHERE invoice_date >= '" + sDate + "' AND invoice_date <= '" + eDate + "' "
    ...
    for (const QVariant& itemValRow : rows) {
        ...
        QString custVal = rowMap.value("customer_name").toString();
        ...
        if (!q.isEmpty()) {
            QString searchTarget = (invNo + " " + vchNo + " " + custVal + " " + itemVal + " " + veh).toLower();
        ...
        auto* itemInv = new QTableWidgetItem(invNo);
    ...
    m_totalInvoicesLabel->setText(QString::number(totInv));
...
void SalesRegisterWidget::onSearchChanged(const QString& query) {
    Q_UNUSED(query);
    populateTable();
}

void SalesRegisterWidget::onDateFilterChanged() {
    populateTable();
}

void SalesRegisterWidget::focusTable() {
...
void SalesRegisterWidget::onTableDoubleClicked(int row, int col) {
    Q_UNUSED(col);
...
void SalesRegisterWidget::onPrintInvoice() {
    int row = m_table->currentRow();
    if (row < 0 || row >= m_table->rowCount()) {
        CustomMessageBox::showWarning(this, "Select Invoice", "Please select an invoice from the table to print.");
        return;
    }

    QString invNo = m_table->item(row, 0)->text();
    QString cust = m_table->item(row, 2)->text();

    QString copyType = PrintCopyOptionsDialog::selectCopyType(PrintCopyOptionsDialog::Mode::Print, invNo, cust, this);
    if (!copyType.isEmpty() && m_printExportCtrl) {
        m_printExportCtrl->print_sales_invoice(invNo, copyType);
    }
}

void SalesRegisterWidget::onExportPdf() {
    int row = m_table->currentRow();
    if (row < 0 || row >= m_table->rowCount()) {
        CustomMessageBox::showWarning(this, "Select Invoice", "Please select an invoice from the table to export as PDF.");
        return;
    }
    ...
    QString copyType = PrintCopyOptionsDialog::selectCopyType(PrintCopyOptionsDialog::Mode::Pdf, invNo, cust, this);
    if (!copyType.isEmpty() && m_printExportCtrl) {
        m_printExportCtrl->export_sales_invoice_pdf(invNo, "", copyType);
    }
}

void SalesRegisterWidget::onExportCsv() {
    QString fileName = QFileDialog::getSaveFileName(this, "Export Sales Register CSV", "SalesRegister.csv", "CSV Files (*.csv)");
    ...
    out << "Invoice No,Date,Customer,Commodity,Bags,Weight Qtl,Rate,Taxable,GST,Total,Vehicle No\n";
    ...
    file.close();
    CustomMessageBox::showInformation(this, "Export Complete", "Sales Register exported to " + fileName);
}
