...
NewLedgerWidget::NewLedgerWidget(QWidget* parent)
    : QWidget(parent)
{
    setupUi();
    populateDropdowns();
...
void NewLedgerWidget::setupSearchableCombo(QComboBox* combo, const QStringList& items) {
    if (!combo) return;
...
void NewLedgerWidget::setComboText(QComboBox* combo, const QString& text) {
    if (!combo) return;
...
QString NewLedgerWidget::getComboText(const QComboBox* combo) const {
    if (!combo) return "";
    return combo->currentText().trimmed();
}

void NewLedgerWidget::setupUi() {
    setAttribute(Qt::WA_StyledBackground, true);
    setStyleSheet("NewLedgerWidget { background-color: #F8FAFC; }");
    ...
    connect(m_cancelBtn, &QPushButton::clicked, this, &NewLedgerWidget::backRequested);
    bottomLayout->addWidget(m_cancelBtn);

    bottomLayout->addStretch();

    m_saveBtn = new QPushButton("Save Ledger Account (Enter)", bottomCard);
    m_saveBtn->setFixedSize(220, 30);
    m_saveBtn->setCursor(Qt::PointingHandCursor);
    m_saveBtn->setStyleSheet("QPushButton { background-color: #2563EB; color: #FFFFFF; border: 1px solid #1D4ED8; border-radius: 5px; font-weight: 800; font-size: 12px; } QPushButton:hover { background-color: #1D4ED8; } QPushButton:focus { border: 2px solid #93C5FD; }");
    connect(m_saveBtn, &QPushButton::clicked, this, &NewLedgerWidget::onSaveClicked);
    ...
    connect(m_nameInput, &QLineEdit::editingFinished, this, &NewLedgerWidget::onNameEditingFinished);
    connect(m_opBalInput, &QLineEdit::textChanged, this, &NewLedgerWidget::updateTotalOpeningBalDisplay);
    connect(m_balTypeCombo, &QComboBox::currentTextChanged, this, &NewLedgerWidget::updateTotalOpeningBalDisplay);
    connect(m_gstinInput, &QLineEdit::textChanged, this, &NewLedgerWidget::onGstinChanged);
    connect(m_stateCombo, &QComboBox::currentTextChanged, this, &NewLedgerWidget::onStateChanged);
...
QWidget* NewLedgerWidget::createHeaderSection() {
    ...
    // Center: Banner Title
    auto* titleLbl = new QLabel("Ledger Creation", card);
...
QWidget* NewLedgerWidget::createSlateSection() {
    auto* card = new QFrame(this);
...
void NewLedgerWidget::populateDropdowns() {
...
void NewLedgerWidget::resetForm() {
    populateDropdowns();
...
void NewLedgerWidget::focusFirstField() {
    if (m_nameInput) {
...
void NewLedgerWidget::showEvent(QShowEvent* event) {
...
void NewLedgerWidget::updateTotalOpeningBalDisplay() {
    ...
    QVariantMap summary = m_partiesModel.get_total_opening_balance_summary(0, amt, bType);
...
void NewLedgerWidget::onNameEditingFinished() {
...
void NewLedgerWidget::onGstinChanged(const QString& text) {
...
void NewLedgerWidget::onStateChanged(const QString& stateName) {
...
void NewLedgerWidget::keyPressEvent(QKeyEvent* event) {
...
bool NewLedgerWidget::eventFilter(QObject* watched, QEvent* event) {
...
void NewLedgerWidget::onSaveClicked() {
    ...
    bool confirmed = CustomMessageBox::question(
        this,
        "CONFIRM LEDGER SAVE",
        QString("Are you sure you want to save & create Ledger Account '%1' under group '%2'?").arg(name, group),
        "Save Ledger",
        "Cancel"
    );
...
void NewLedgerWidget::executeSave() {
    QString name = m_nameInput->text().trimmed();
    ...
    bool success = m_partiesModel.add_ledger_extended(data);

    if (success) {
        CustomMessageBox::information(this, "Success", QString("Ledger Account '%1' created successfully.").arg(name));
        emit savedSuccess();
        emit backRequested();
    } else {
        CustomMessageBox::critical(this, "Error", "Failed to save ledger account into database.");
    }
}
